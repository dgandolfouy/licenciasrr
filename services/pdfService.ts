...
export const generateEmployeeReport = async (employee: Employee) => {
  const doc = new jsPDF();
  const summary = getLeaveDaysSummary(employee);
  ...
  doc.text('FICHA DE LICENCIAS', 20, 60);
  ...
  autoTable(doc, {
    startY: 118,
    head: [['Días Generados', 'Fijos Acordados', 'Gozados', 'Saldo Disponible']],
    body: [[
        `${summary.totalGenerated} d`,
        `${summary.fixedDeductions} d`,
        `${summary.takenDays} d`,
        // Fix: Cast fontStyle to 'bold' to match the FontStyle union type in jspdf-autotable
  ...
  const history = employee.leaveRecords
  ...
  autoTable(doc, {
    ...
    head: [['Fecha', 'Cant.', 'Tipo', 'Detalle']],
    body: history.length > 0 ? history : [['-', '-', 'Sin registros', '-']],
    theme: 'striped',
...
export const generateGeneralReport = async (employees: Employee[]) => {
    ...
    doc.setTextColor('#1d1d1b');
    doc.setFont('helvetica', 'bold');
    doc.text('REPORTE CONSOLIDADO DE LICENCIAS - NÓMINA GENERAL', 20, 60);
    
    const body = employees
        .sort((a, b) => a.lastName.localeCompare(b.lastName))
        .map(emp => {
            const sum = getLeaveDaysSummary(emp);
            return [
    ...
    autoTable(doc, {
        startY: 68,
        head: [['Colaborador', 'Documento', 'Totales', 'Fijos', 'Gozados', 'Saldo']],
        body: body,
        theme: 'grid',
